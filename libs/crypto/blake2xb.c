...
int blake2xb_final_xor( blake2xb_state *S, void *out, size_t outlen) {
  ...
  for (i = 0; outlen > 0; ++i) {
    ...
    /* Process key if needed */
    blake2b_update(C, root, BLAKE2B_OUTBYTES);
    if (blake2b_final(C, tmp, block_size) < 0 ) {
        return -1;
    }
    for(int j=0;j<block_size;j++) {
    	*((uint8_t *)out + i * BLAKE2B_OUTBYTES + j) ^= tmp[j];
    }
    outlen -= block_size;
  }
  secure_zero_memory(tmp, BLAKE2B_OUTBYTES);
  secure_zero_memory(root, sizeof(root));
  secure_zero_memory(P, sizeof(P));
  secure_zero_memory(C, sizeof(C));
  /* Put blake2xb in an invalid state? cf. blake2s_is_lastblock */
...
int blake2xb_xor(void *out, size_t outlen, const void *in, size_t inlen, const void *key, size_t keylen)
{
  blake2xb_state S[1];
  ...
  /* Compute the root node of the tree and the final hash using the counter construction */
  return blake2xb_final_xor(S, out, outlen);
}
